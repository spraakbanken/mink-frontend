...
const GENERATED_COMMENT = "# Generated by Mink\n";
...
export function makeConfig(id: string, options: ConfigOptions): string {
  ...
  const yaml = Yaml.dump(config as SparvConfig, { noArrayIndent: true });
  // Mark YAML as generated from standard options.
  return asGenerated(true, yaml);
}
...
/** Flag or unflag config YAML as generated from standard options form. */
export function asGenerated(generated: boolean, yaml: string) {
  if (generated && !isGenerated(yaml)) yaml = GENERATED_COMMENT + yaml;
  if (!generated && isGenerated(yaml))
    yaml = yaml.replace(GENERATED_COMMENT, "");
  return yaml;
}

/** Detect if a config YAML was saved from the standard options form. */
export const isGenerated = (yaml: string): boolean =>
  yaml.indexOf(GENERATED_COMMENT) === 0;
